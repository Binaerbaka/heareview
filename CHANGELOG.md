...
### 新增
- 最终 Review：课堂结束后从完整确认字幕生成结构化复习资料（课堂总览 / 核心概念 / 老师反复强调的重点 / 专业术语 / 易混淆点）
- 长课堂分段生成：超过阈值时按时间顺序分块分析，再合并为最终 Review，完整覆盖整堂课
- 最终 Review 写入 Markdown 导出（`## Final Review`）与 JSON 导出
- 共享 LLM 请求队列：课堂要点 / 字幕翻译 / 最终 Review 串行请求同一服务

### 变更
- 统一版本号为 v0.7.0
- 合并为一个通用 LLM 请求函数 `requestLlmJson`，减少重复代码
- 最终 Review 的课堂总览改用段落显示，不再放进列表

### 修复
- 修复最终 Review 只取末尾 60000 字符、导致长课堂前半段丢失的问题

## [0.6.3] - 2026-09-16

> 整合 0.6.1–0.6.2.1 的功能：确认字幕翻译、可调整布局、临时字幕翻译。

### 新增
- 确认字幕翻译（v0.6.1）：与课堂要点共用 LLM 配置，支持 Ollama / DeepSeek / OpenAI-compatible
- 翻译方向切换（EN → 中 / 中 → EN）；关闭翻译保留缓存，新课堂清空缓存
- 可调整布局（v0.6.2）：拖动分隔线调整设置栏宽度与总结面板高度，尺寸保存在本机
- 临时字幕翻译（v0.6.3）：只翻译 `buffer_transcription`，约 1 秒静止后更新，持续变化时约每 3 秒至少一次
- Markdown 导出包含已确认字幕译文（临时译文不参与导出）

### 修复
- 修复翻译结果重渲染时缓存 key 失配导致的“翻译成功却不显示”
- 修复长课堂自动总结失效与失败重试过频
- 临时翻译与确认字幕翻译串行，避免同时请求同一个 LLM 服务
- 临时翻译 cycle 正确结束；方向切换时使在飞的临时翻译结果失效
...
### 新增
- 右侧“实时课堂要点”面板：基于已确认字幕调用外部 LLM 生成阶段总结
- 支持 Ollama（本机）、DeepSeek API、OpenAI-compatible 三类服务
- 自动生成：新增约 420 个已确认字符且距上次请求至少 45 秒时触发，也可手动“生成当前总结”
- 总结写入 `sessionData.summary`，Markdown / JSON 导出均包含总结内容
- API Key 只保留在当前页面，不写入 localStorage、导出 JSON 或字幕文件

### 变更
- 将总结功能直接整合进 `frontend/app.js`、`index.html`、`style.css`，不再使用独立扩展文件
- 自动生成改由字幕渲染完成后的函数调用触发，替代独立文件的 `MutationObserver`
- 面板固定显示在右侧信息栏顶部

### 修复
- 修复长课堂自动总结失效：改用已确认字幕总字符数判断阈值，不再受 8000 字符上下文截断影响
- 修复 LLM 不可用时被高频重试：发起请求时即记录时间，限制自动请求频率
- 新课堂开始时重置要点面板与总结状态

### 说明
- 未配置 LLM 或服务不可用时，原有字幕功能完全不受影响
- Ollama 需本机运行，默认地址 `http://127.0.0.1:11434`
